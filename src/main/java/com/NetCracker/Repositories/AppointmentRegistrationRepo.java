...
//            @Query("select a from AppointmentRegistration a where a.doctor.lastName = 'Фролов В.В.' order by a.id asc")
    ...
    @Query("select a from AppointmentRegistration a, Doctor d, Patient p where a.doctor.id = d.id and a.patient.id = p.id and p.user.lastName = :pat and d.lastName = :doc and a.start = (select min(aa.start) from AppointmentRegistration aa, Doctor dd, Patient pp where aa.doctor.id = dd.id and aa.patient.id = pp.id and pp.user.lastName = :pat and dd.lastName = :doc)")

...
public interface DoctorScheduleRepository extends JpaRepository<DoctorSchedule, Long> {
    @Query(value = "SELECT d FROM DoctorSchedule d WHERE d.id IN :doctorIds"/*, nativeQuery = true*/)
    Set<DoctorSchedule> findByRelatedDoctor(@Param("doctorIds") Collection<Long> doctorIds) throws DataAccessException;

    @Query(value = "SELECT d FROM DoctorSchedule d WHERE d.id = :doctorId"/*, nativeQuery = true*/)
